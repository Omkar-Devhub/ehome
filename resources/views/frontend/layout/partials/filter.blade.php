<div id="filter-section">
    ...
    <form id="propertyFilterForm"
        action="{{ route('properties.index', ['adType' => $adType->name, 'propertyType' => $propertyTypeSlug]) }}"
        method="GET">
        <div class="mb-2">
            <select class="shadow-none form-select county-dropdown" name="county_id">
                <option value="" selected="">Select County</option>
                @foreach ($counties as $county)
                    <option value="{{ $county->id }}" {{ request('county_id') == $county->id ? 'selected' : '' }}>
                        {{ Str::title($county->name) }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-2">
...
@section('script')
    <script>
        ...
        $(document).ready(function() {
            $(document).on("change", ".county-dropdown", function() {
                var county_id = $(this).val();
                var form = $(this).closest("form"); // Get the parent form
                var areaDropdown = form.find(".area-dropdown"); // Find area dropdown in the same form
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        areaDropdown.find("option:not(:first)").remove(); // Clear old options
        ...
        document.getElementById('propertyFilterForm').addEventListener('submit', function(event) {
            ...
            const url =
                "{{ route('properties.index', ['adType' => $adType->name, 'propertyType' => $propertyTypeSlug]) }}" +
