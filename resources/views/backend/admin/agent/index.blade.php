                    ...
                    <div class="card-header">
                        <div class="card-title">
                            <button type="button" class="btn btn-default btn-sm"
                                onclick="window.location.href ='{{ route('admin.vendors') }}'">Reset</button>
                        </div>
                                ...
                                @if ($agents->count() > 0)
                                    @foreach ($agents as $agent)
                                        <tr>
                                            ...
                                            <td>{{ $agent->email }}</td>
                                            <td>{{ $agent->phone }}</td>
                                            <td>{{ $agent->office_address }}, {{ Str::ucfirst($agent->area->name) }},
                                                {{ Str::ucfirst($agent->county->name) }},
                                                {{ $agent->eircode }}</td>
                                            <td>
                                                @if ($agent->status == 1)
                                            ...
                                            <td>
                                                ...
                                                <a href="{{ route('admin.agents.delete', $agent->id) }}"
